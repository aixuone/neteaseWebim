...
window.yunXin.WB = new window.Vue({
  ...
  methods: {
    ...
    drawcan:function(){
      ...
      var canvas = document.getElementById('wbCan')
      var canvasContext = canvas.getContext('2d')

          canvasContext.drawImage(video, 0, 0, 640, 480)
          this.srcImgForDraw =  canvas.toDataURL('image/jpeg')
          // this.seeImgForDraw = true;

           $.ajax({
                    type : "POST",
                    url : 'http://smart.tygps.com/im/whiteboard/image/upload',
                    data:{
                      imagefile: that.srcImgForDraw
                    },
                    success : function(data) {

                            if (data.success=="true") 
                              that.seeImgForDraw = true;
                            else 
                              alert(data.message);
    ...
    ajaxForImg: function(username){
      var that = this;
         $.ajax({
                    type : "GET",
                    url : 'http://smart.tygps.com/im/whiteboard/image/'+username,
                    
                    success : function(data) {
　　　　　　　　　　　　　　that.srcImgForDraw = data;
                            if (that.srcImgForDraw!='') that.seeImgForDraw = true;
                    },
                    error : function(err){
                            that.srcImgForDraw = '';
                            alert("白板截图图片加载错误！错误原因:"+err);
